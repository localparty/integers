...
- **Step 1**: Use the 5-axiom CBB system definition (research/213, paper12/27-anchor-document.md) to directly characterize $D_N$ via the critical operator $L̂ = \log R̂$ on $H_R \subset H_1$
...
| Priority | File | Why |
|:--|:--|:--|
| 1 (MUST) | `paper13-rh/preprint/00-proof-skeleton.md` | The 6-layer chain + Layer 1 description |
| 2 (MUST) | `paper13-rh/preprint/sections-01-05.md` | Setup, BC algebra, GNS, Meyer distributional, CCM interface |
| 3 (MUST) | `paper13-rh/preprint/sections-06-10.md` | CCM operators, ITPFI factorization, Bögli compactness, Hurwitz |
| 4 (MUST) | `paper13-rh/preprint/PROOF-CHAIN.md` | The proof-chain summary file |
| 5 (HIGH) | `paper13-rh/strategy/28-all-gaps-closed.md` | The closing artifact + Layer status |
| 6 (HIGH) | `paper13-rh/strategy/11-ccm-itpfi-programme.md` + `12-ccm-itpfi-current-state.md` | CCM programme state history |
| 7 (HIGH) | `paper13-rh/strategy/05-literature-leads.md` | Alternative spectral-triple candidates; the literature survey |
| 8 (HIGH) | `paper13-rh/strategy/02-where-we-stand.md` + `15-final-state-rh.md` | Where the programme converged and why CCM became the foundation |
...
| 10 (MED) | `paper08-yang-mills/h4-capacitor-bypass/synthesis/wave1-synthesis.md` | H4 run's synthesis for pattern-learning (Author format compliance, coordination insights) |
| 11 (MED) | `paper08-yang-mills/h4-capacitor-bypass/capacitor/updates/wave1-cell-fills.md` | 5 new cells + K-8 from H4 run |
| 12 (MED) | `paper12/27-anchor-document.md` | CBB system 5-axiom definition (Alt-3 candidate) |
| 13 (MED) | `paper12/research/214-the-method-six-patterns.md` | Six Patterns method (Always-include) |
| 14 (LOW) | `paper13-rh/strategy/10-state-after-18-kills.md` | Prior kill list — none are bypass kills but pattern-useful |

**Plus (runtime)** — primary-source verification for any CCM alternatives proposed. If an Author cites "Bost-Connes 1995" or "Connes-Marcolli 2008", they MUST fetch the paper via WebFetch or read from `paper13-rh/math-referee/` cached PDFs if available, and produce verbatim supporting quotes.
...
1. **BYPASS FOUND** — compound bypass produces a Layer-1-replacement that:
   ...
   → **RH CCM-UNCONDITIONAL.** Paper 13 Layer 1 has an alternative foundation. Phase 2's Tier 1 CCM verification becomes optional. Update capacitor with new filled cells. Cascade-note the finding to `paper13-rh/preprint/00-proof-skeleton.md` for the next preprint revision.
...
**Output directory:** `paper13-rh/ccm-bypass/` (sibling pattern like `paper08-yang-mills/h4-capacitor-bypass/`). Do NOT write to existing `paper13-rh/preprint/` or `paper13-rh/strategy/` files. CASCADE-notes to `preprint/00-proof-skeleton.md` are allowed via explicit CASCADE entries, not blackboard bootstrap.

**Blackboard:** create fresh at `${output-directory}/BLACKBOARD.md`.

**Chain state:** inherit from `paper13-rh/preprint/00-proof-skeleton.md` (Layer statuses). Only Layer 1 is in play for this run.
