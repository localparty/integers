...
<!-- Vocabulary canon note (Intervention 8b, 2026-04-15, aggressive migration applied): this file uses "5-dimensional", "5D", "fifth dimension" as subject-matter language. Per strategy/north-star.md §0.10 (Vocabulary Canon), canonical phrasing is "4+1 coordinate structure" / "M⁵" / "internal phase" / "S¹ coordinate". Inline strikethrough migrations applied per Intervention 8b. -->
...
The ~~QG5D~~ Integers<!-- legacy 2026-04-15 Intervention 8b §0.10 §0.1: "QG5D" (programme shorthand) → "Integers" --> programme's central claim is that this appearance of independence is a projection artifact.

The six conjectures appear unrelated because they are being studied from INSIDE six different projections of a single object. Each community has access to one projection direction and cannot see the others. From inside any single projection, the other projections look entirely foreign — different languages, different objects, different techniques. But the object casting all six shadows is the same: the ~~5-dimensional geometry~~ 4+1 coordinate geometry (M⁵)<!-- legacy 2026-04-15 Intervention 8b §0.10: "5-dimensional geometry" → "4+1 coordinate geometry (M⁵)" --> $M^5 = M^4 \times S^1$.

This is not a metaphor. It is a precise mathematical claim: each of the six Clay problems is the image of a specific smooth map (or adjunction) out of $M^5$. The maps have different domains of definition, preserve different invariants, and forget different structure — which is exactly why the shadows look different. But the pre-image of every shadow is the same ~~5D manifold~~ M⁵ manifold<!-- legacy 2026-04-15 Intervention 8b §0.10: "5D manifold" → "M⁵ manifold" -->.

### 19.2 RH as a projection: Branches A + D + E

The Riemann Hypothesis is the statement $\zeta(s) = 0 \Rightarrow \text{Re}(s) = 1/2$. In the ~~QG5D~~ 4+1<!-- legacy 2026-04-15 Intervention 8b §0.10 §0.1: "QG5D framework" (derivation shorthand) → "4+1 framework" --> framework, $\zeta(s)$ is the partition function of the Bost-Connes (BC) algebra at inverse temperature $\beta = s$. The zeros of $\zeta(s)$ are the eigenvalues of a specific self-adjoint operator $D_\infty$ on the Hilbert space $H_R$ of Paper 13. The question $\text{Re}(s) = 1/2$ becomes: are the eigenvalues of $D_\infty$ real?
...
In the ~~QG5D~~ 4+1<!-- legacy 2026-04-15 Intervention 8b §0.10 §0.1: "QG5D framework" (derivation shorthand) → "4+1 framework" --> framework, the L-function $L(E, s)$ is a component of the Bost-Connes partition function restricted to the CM (complex multiplication) points of $E$. The Hecke operators $\mu_p$ act on the BC algebra and, when restricted to CM points, produce the Frobenius data that defines $L(E, s)$. The rank of $E$ is the multiplicity of the zero of $L(E, s)$ at $s = 1$, which is the number of degenerate KMS states at $\beta = 1$ — states that don't break the symmetry.
...
The P vs NP problem asks: is there a polynomial-time algorithm for every problem whose solution can be verified in polynomial time? In the ~~QG5D~~ 4+1<!-- legacy 2026-04-15 Intervention 8b §0.10 §0.1: "QG5D framework" (derivation shorthand) → "4+1 framework" --> framework (Paper 28), P vs NP is the question of whether a specific type III$_1$ von Neumann factor is FULL — whether every bounded operator approximable by local operators is approximable by GLOBAL operators.
...
The ~~QG5D~~ Integers<!-- legacy 2026-04-15 Intervention 8b §0.10 §0.1: "QG5D" (programme shorthand) → "Integers" --> programme has identified 10 such question-types. This section names each face, identifies its question, maps the conjecture that interrogates it, and explains WHY this face is a natural category for $S^1$.
...
The complete table is in `integers/paper12-cbb-system/research/23-framework-predictions-master-table.md`. A representative sample across the 5 sectors:
...
2. The ~~QG5D~~ 4+1<!-- legacy 2026-04-15 Intervention 8b §0.10 §0.1: "QG5D framework" (derivation shorthand) → "4+1 framework" --> framework produces $P_E(M^5) = v_\text{obs}$ exactly (at sub-percent precision for all 36 components)
...
The ~~QG5D~~ 4+1<!-- legacy 2026-04-15 Intervention 8b §0.10 §0.1: "QG5D framework" (derivation shorthand) → "4+1 framework" --> framework has zero free parameters.
...
The 36 sub-percent matches confirm: no adjustment was needed. The geometric origin of the constants — the Riemann zeros of the BC algebra's partition function, organized by the operator dictionary $\hat{L} = \log \hat{R}$ — reproduces reality without modification. This is the ~~QG5D~~ 4+1<!-- legacy 2026-04-15 Intervention 8b §0.10 §0.1: "QG5D framework" (derivation shorthand) → "4+1 framework" --> framework's strongest empirical claim, and it stands unrefuted as of April 14, 2026.
...
| §20 | `strategy/the-picture-of-the-ecircle.md`; `integers/paper60-geometry-of-circle/00-table-of-contents.md` |
| §21 | `strategy/the-weakest-links.md`; `strategy/the-ring.md` |
| §22 | `integers/paper01-qg5d/PROOF-CHAIN.md`; `strategy/inner-rings/README.md` |
| §23 | `integers/paper12-cbb-system/research/23-framework-predictions-master-table.md`; Paper 1 Branch E |
