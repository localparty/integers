...
| **6.5 Cross-sector dual appearances: 11 confirmed** | Full table of γ_n with dual (or more) physical appearances: γ_1 (CC + R_obs + mass gap), γ_2 (CC + Higgs + BC SSB), γ_3 (CC + m_t), γ_4 (m_u Yukawa + 1/α + 1st-gen mixing splitter), γ_5 (inflation + sin θ_13 CKM + DM + hierarchy), γ_6 (m_H + N_eff + CKM/PMNS), γ_7 (m_τ + t_0), γ_8 (m_t + m_τ), γ_9 (n_s numerator), γ_11 (H_0 + 1/α_3 + m_Z + PMNS θ_23 — **the γ_11 triple**: three observables from the same zero via different KK normalisations: 4/π, 1/(2π), 1/γ_E), γ_13 (m_W + Y_p). Refined principle: **ground-state γ_1 is universal; cross-sector γ_n for n ≥ 2 each indexes specific physics**. | Non-trivial framework predictions tested by independent observables; round 3 expands from 4 to 11; round 5 upgrades γ_11 to triple | research/74, research/91, dictionary §9 |
