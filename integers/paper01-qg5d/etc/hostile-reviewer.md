<!-- Vocabulary canon note (Intervention 8b, 2026-04-15, aggressive migration applied): this file is an operational hostile-review prompt scaffold. Per strategy/north-star.md §0.10 (Vocabulary Canon), canonical phrasing is "4+1 coordinate structure" / "M⁵" / "M⁵ gravity" / "internal phase" / "S¹ coordinate". Inline strikethrough migrations applied per Intervention 8b to thesis references. The hostile-reviewer language itself is an adversarial persona prompt — canonical form preferred in the paper's prose, not in this scaffold. -->
...
- `integers/paper01-qg5d/etc/in-depth-review.md` — the most recent hostile review with
  fixing plan (all fixes now applied)
- `etc/integers/paper01-qg5d/17-hostile-reviewer.md` — the original hostile review
  (7 critical issues found; 3 fully fixed, 3 partially fixed)
- `etc/integers/paper01-qg5d/00-project-master.md` — project master document with
  claims hierarchy and session history

### What to read

Read ALL files in `integers/paper01-qg5d/` — every section, every appendix (A through Z),
and `integers/paper01-qg5d/abstract.md`. Read the prior audits listed above. Read
`integers/paper01-qg5d/etc/refs.bib`.
...
Before flagging an issue, check `integers/paper01-qg5d/etc/in-depth-review.md` to see
...
Check that every citation has a corresponding entry in `integers/paper01-qg5d/etc/refs.bib`.
...
Produce your review in `integers/paper01-qg5d/etc/in-depth-review.md` with sections:
