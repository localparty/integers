...
| **Shared foundations** | Hilbert space; Wigner's theorem; π₁(SO(d)) = ℤ₂ | Same (not derived from the 5D framework; inputs also to standard proof) |
| **Additional input** | Lorentz invariance; locality; positive energy; microcausality | e-phase coupling [Theorem T3 per strategy/paper1-audit/derivation-chains.md §T3; Step 3 of App. B derivation] |
<!-- legacy 2024 table labels, reclassified 2026-04-15 per strategy/paper1-audit/ -->
<!-- Original row 1: "Same (not derived from 5D postulate)" — the π₁(SO(d)) input is a standard mathematical fact, not a programme postulate; wording sharpened. -->
<!-- Original row 2: "e-phase coupling postulate (Step 3)" — e-phase coupling is now Theorem T3 (U(1) isometry of S¹ + Noether). -->
