...
<!-- Vocabulary canon note (Intervention 8b, 2026-04-15, aggressive migration applied): this file is the heaviest-hit appendix (49 occurrences of "5D" / "five-dimensional" / "fifth dimension"). Per strategy/north-star.md §0.10 (Vocabulary Canon), canonical phrasing is "4+1 coordinate structure" / "M⁵" / "M⁵ gravity" / "internal phase" / "S¹ coordinate". Inline strikethrough migrations applied per Intervention 8b to thesis sentences and high-salience passages; technical computation body retains "5D" / "5D KK theory" as subject-matter shorthand for the Goroff-Sagnotti / dim-reg comparison — canonical form is "M⁵ KK theory". -->
...
- *Numerical:* `integers/paper01-qg5d/code/seeley-dewitt/results.txt` fits the heat-kernel
...
- `integers/paper01-qg5d/code/seeley-dewitt/results.txt` — a₂ = a₄ = 0 to 9 s.f.
- `integers/paper01-qg5d/code/three-graviton-vertex/results.txt` (PART 7) — `E_L(−j; Q) = 0`
  via `1/Γ(−j)` for j = 1..7 and representative positive-definite Q.
- `integers/paper11a-all-orders-finiteness/code/bootstrap_L4_verify.py` — all-orders bootstrap through
...
(c) **Weyl anomaly of the full orbifold KK tower — CLOSED (2026-04-14).**
    ...
    The computation is certified by `integers/paper01-qg5d/code/a2-graviphoton-radion/results.txt`
