...
> **RING-MODE EXCEPTION (added 2026-04-14)**: when this file is cited as the `toolkit` in a run invocation (e.g., `30-ring-traversal-run.md`), it IS the runtime toolkit for that run. **Read it fully end-to-end at bootstrap.** Its §A (Six Patterns method) and §E (anchor document) always-includes are actionable as spawn-context injections for every Author / Critic / Synthesis dispatch. The "reference index, NOT a runtime file" self-identification below describes this file's role when BUILDING new programme-specific toolkits; when invoked DIRECTLY via a run file's toolkit line, this file serves BOTH roles. Do not defer reading based on the self-identification — the invocation elevates it to runtime status.
