...
**Theorem F.1.** Let a Euclidean gauge theory on $\mathbb{R}^4$ satisfy:
...
Then the reconstructed Hamiltonian $H$ has a mass gap
$\Delta \geq c \sqrt{\sigma}$ for a dimensionless constant $c > 0$.
...
Fredenhagen and Marcu (1986) provided a rigorous criterion for
...
with $c \approx 2$ from the CP$^2$ string calculation. The mass gap is
a direct consequence of the string tension, which is a direct
consequence of the area law, which is a direct consequence of
$\mathbb{CP}^2$ topology.
