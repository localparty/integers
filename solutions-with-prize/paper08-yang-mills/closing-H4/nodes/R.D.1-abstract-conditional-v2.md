...
>
> <!-- W2-CLEANUP (2026-04-11, Referee Finding 1 MAJOR): The 4-bullet block below (Routes A/B/C/D with pre-Wave-1 probabilities $p \approx 0.50, 0.30, 0.25, 0.99$) is STALE. Per H4 closure Wave 1 adjudication (`paper08-yang-mills/closing-H4/closure/closure-digest.md`, 2026-04-11): Route A decomposed into R.A.1a + R.A.1b ($p \approx 0.10$, both deferred pending external unlocks); Route B COLLAPSED into Route A (reduces to the same identity-theorem mechanism in spectral-triple language, plus a Wrong-space category error at CCM 2025 → YM); Route C was KILLED (§F K-2 — Connes 2007 Séminaire Poincaré §5 eq. (24) defines the spectral action at the *classical* level as bare actions at $\Lambda$, NOT running flow; Vassilevich 2003 confirms $\beta_0 = 11N/3$ is a counter-term coefficient, not a running-coupling slope); Route D produced the editorial shipping form (this node, v2). During W2 editorial merge into Paper 8's preprint, REPLACE the 4-bullet block with a single paragraph summarising Wave 1's outcome and STRIP runner-ritual phrases ("Runner $p \approx 0.99$", "first-class per v3 patch I-5", "The HONEST-STALL option", "~70% completeness"). Cite `closing-H4/closure/closure-digest.md §Kills` for the kills and `§The LOCK` for the 9/10 cross-node structural LOCK on the attack surface. -->
>
...
>
> <!-- W2-CLEANUP (2026-04-11, Referee Finding 1 MAJOR): Next sentence says "Three routes are attacking it" — post-Wave-1, the three mathematical routes were adjudicated and did NOT close H4. During W2 editorial merge, REPLACE "Three routes are attacking it. Paper 8 ships either way." with "Paper 8 ships either way via the editorial shipping form developed in this paper." See `closing-H4/closure/closure-digest.md §Programme state` + `§The closure statement`. -->
>
...
>
> <!-- W2-CLEANUP (2026-04-11, Referee Finding 1 MAJOR): The paragraph below cites pre-Wave-1 probabilities ($p \approx 0.50, 0.30, 0.25$) and joint $P \approx 0.74$. Post-Wave-1 (H4 closure programme 2026-04-11): $P(A \vee B \vee C) \approx 0.10$; all three mathematical routes adjudicated negatively. During W2 editorial merge into Paper 8's preprint, REPLACE this paragraph with: "The mathematical closure routes A/B/C were adjudicated in the H4 closure programme (`paper08-yang-mills/closing-H4/`, Wave 1, 2026-04-11) and did not close H4. Route A's identity-theorem mechanism hits a formal-series obstruction on the perturbative side (decomposed into two sub-problems, both comparable in difficulty to Borel summability for 4D SU(N) YM and deferred pending external unlocks). Route B collapsed into Route A as a reformulation in CCM 2025 spectral-triple language plus a Wrong-space category error (target data are Taylor coefficients of an analytic function; CCM machinery requires zeros of an entire function). Route C was killed by Connes 2007 Séminaire Poincaré X §5 eq. (24): the spectral action is defined at the classical level as bare actions at $\Lambda$, not running flow. Paper 8's Theorem 1 Clause (C) carries H4 as a named standing conditional in the W7-14 §5.3 mildest form; the framework's empirical content is independent of H4 and unaffected." See `closing-H4/closure/closure-digest.md §Kills` for K-1 (CCM port) + K-2 (spectral action) and `§The LOCK` for the 9/10 cross-node structural lock on the Taylor-coefficient attack surface. -->
>
