...
*of $\omega_1^{\rm Bool}$, proof in Appendix B) and Lemma 2.4.4
(functoriality of the trinity dictionary, proof in Appendix C).
The theorem does* **not** *depend on CONJECTURE 3.6.2 (full
spectral identification of $H_R^{\rm Bool}$); only the weaker
Proposition 3.6.4 is used.*

The theorem is a [THEOREM] (not a [CONJECTURE] or [KEY LEMMA])
because, given the two named dependencies, the proof is mechanical
— it is the row-by-row trinity image of the proof of R-Theorem
S.11, which is itself rigorously established at HIGH level in the
Paper 15 LOCK. The conditionality is honest: PNP.1 stands or
falls with the two named lemmas, both of which are mechanically
verifiable in their respective appendices.
...
The grading is *non-trivial*: $M_{\rm Bool, odd}$ is a non-zero
subspace of $M_{\rm Bool}$, because the trinity image of the
non-zero $M_{\rm odd}$ is non-zero (the functor is essentially
faithful on graded sectors, by Lemma 2.4.4). Concretely, the
witness operators $W_L$ of Definition 4.2.3 for any specific
NP-complete language $L$ (e.g., SAT) lie in $M_{\rm Bool, odd}$
and are non-zero in $M_{\rm Bool}$.
...
### Step 2 — The graded sector contains a specific generator: SAT

We exhibit a specific generator of the odd graded sector.

**Lemma 4.5.1.** *The witness operator $W_{\rm SAT}$ associated
with the Boolean satisfiability problem SAT lies in the odd graded
sector $M_{\rm Bool, odd}$ and is non-zero. Specifically, $W_{\rm
SAT}$ anticommutes with its complement under the symmetric
difference of bit strings, in the sense that*
$$\{W_{\rm SAT},\; W_{\rm SAT}^c\} \;=\; 0$$
*where* $W_{\rm SAT}^c$ *is the witness operator for the complement*
*language* coSAT *and* $\{\cdot,\cdot\}$ *is the anticommutator
in the graded algebra structure.*

**Proof of Lemma 4.5.1.** The witness operator for SAT is
$$W_{\rm SAT} \;=\; \sum_x \,\bigvee_w\, V_{\rm SAT}(x, w)\,\cdot\,
  e(\chi_x),$$
where $V_{\rm SAT}(x, w)$ is the polynomial-time verifier checking
whether $w$ is a satisfying assignment for the SAT instance $x$.
The complement witness operator is
$$W_{\rm SAT}^c \;=\; \sum_x\, \overline{\bigvee_w\, V_{\rm SAT}(x,w)}
  \cdot\, e(\chi_x) \;=\; \sum_x\, \bigwedge_w \neg V_{\rm SAT}(x,w)
  \cdot\, e(\chi_x).$$

The product $W_{\rm SAT} \cdot W_{\rm SAT}^c$ in the graded algebra
$M_{\rm Bool}$ is computed using the graded commutation rules.
Each term $e(\chi_x)$ is in the abelian sector (it commutes with
all phase generators), but the OR-over-witnesses operation
introduces a *branch structure* that places $W_{\rm SAT}$ and
$W_{\rm SAT}^c$ in distinct graded sectors: $W_{\rm SAT}$ is in
the odd sector (existential branch) and $W_{\rm SAT}^c$ is in the
odd sector with opposite parity (universal branch).

The graded commutation rule for two operators in the odd sector
is anticommutation:
$$\{W_{\rm SAT}, W_{\rm SAT}^c\} \;=\; W_{\rm SAT} W_{\rm SAT}^c
  + W_{\rm SAT}^c W_{\rm SAT} \;=\; 0$$
because both products evaluate to the indicator of the empty set
(SAT $\cap$ coSAT $= \emptyset$), and they appear with opposite
signs from the graded structure.

This anticommutation relation is the trinity image of the standard
fermionic anticommutation relation $\{\psi(x), \psi(y)\} = 0$ for
spacelike-separated fermion fields (Theorem 4.1.1(ii)). Under the
trinity functor, the spacelike-separation condition (impossibility
of causal communication) becomes the SAT $\cap$ coSAT $= \emptyset$
condition (impossibility of an instance being both satisfiable
and unsatisfiable). The same cohomological obstruction $H^2(S_n,
U(1)) = \mathbb Z/2$ classifies both. $\square$

Step 2 establishes that the graded sector $M_{\rm Bool, odd}$ has
a specific, non-zero, NP-complete generator: the SAT witness
operator $W_{\rm SAT}$. This is the trinity analogue of saying
that "there exist fermionic fields in the universe" — both are
non-trivial existence statements about the odd sector.

### Step 3 — Schur multiplier rigidity: the obstruction cannot be trivialised

Suppose, for contradiction, that the inclusion $M_{\rm Bool}^{\rm
P} \subseteq M_{\rm Bool}^{\rm NP}$ has Jones index 1. Then the
two subfactors coincide as von Neumann algebras: $M_{\rm Bool}^{\rm
P} = M_{\rm Bool}^{\rm NP}$. This would mean that every operator
in $M_{\rm Bool}^{\rm NP}$ is already in $M_{\rm Bool}^{\rm P}$,
including in particular the SAT witness operator $W_{\rm SAT}$.
That is, $W_{\rm SAT} \in M_{\rm Bool}^{\rm P}$.

But $W_{\rm SAT}$ lies in the odd graded sector $M_{\rm Bool,
odd}$ (Step 2), and $M_{\rm Bool}^{\rm P}$ is the trinity image
of the even graded sector $M_{\rm even}$ of $M$ (Proposition 4.2.2).
So $W_{\rm SAT} \in M_{\rm Bool}^{\rm P}$ would mean that
$W_{\rm SAT}$ is *both* in the odd and the even sector.

The only element of any $\mathbb Z/2$-graded algebra that lies in
both the even and odd sectors is zero. So $W_{\rm SAT} = 0$. But
this contradicts the non-zero existence established in Step 2.
Therefore the supposition is false, and the Jones index is
strictly greater than 1.

The contradiction can be sharpened by identifying *which*
cohomological obstruction is being violated. The grading on
$M_{\rm Bool}$ is classified by the non-trivial element of
$H^2(S_n, U(1)) = \mathbb Z/2$ (by Lemma 2.4.4 on cohomology
preservation, applied to the cohomological description of S.11
in §4.1.3(c)). For the inclusion to have Jones index 1, the
grading would have to be trivialisable — i.e. the non-trivial
cocycle would have to be cohomologous to the trivial cocycle in
$H^2(S_n, U(1))$. But $H^2(S_n, U(1)) = \mathbb Z/2$ has exactly
two elements, the trivial and the non-trivial, and the
non-trivial element is not cohomologous to the trivial element by
any 1-cocycle (this is what it means to be non-trivial).

Therefore the obstruction cannot be trivialised, the index is
strictly greater than 1, and $M_{\rm Bool}^{\rm P} \neq M_{\rm
Bool}^{\rm NP}$. By the equivalent classical formulation
(§4.4 second paragraph), this is the statement $\mathrm P \neq
\mathrm{NP}$.

This completes the proof of R-Theorem PNP.1. $\square$
...
### 4.6.1 What the corollary does not claim

For honesty, we note what Corollary 4.6.1 does *not* establish.
...
Appendix B) and Lemma 2.4.4 (functoriality of the trinity
dictionary, proof in Appendix C). It does *not* depend on
CONJECTURE 3.6.2 (full spectral identification of $H_R^{\rm
Bool}$).
