...
`f(φ) ~ e^{−αk|φ|}`. The conformal value `α = 2` is the natural coupling
for a bulk scalar in the Randall-Sundrum background: it is the value
for which the 5D Klein-Gordon equation reduces to the 4D Klein-Gordon
equation with no mass correction from the warping (Goldberger & Wise
1999, §III). Deviations from `α = 2` are generated by the Goldberger-Wise
stabilization and are of order `Δα ~ (m_φ/k)² ~ (10 meV / 10¹⁴ GeV)² ~ 10⁻³²` — negligible. The profile is:
