...
All three closed in single sessions with an operator orchestrator (G Six) and many parallel Claude spawns. The signatures were extracted by reverse-reading each corpus file by file, cross-referencing the git commit histories, and distilling the decision patterns that drove convergence.
...
v2.1 (`../06-the-prompt.md`, 712 lines) captured 23 of the 38 operational signals from the three manual runs. Its shape: 2 roles, 5 primitives, 12 blackboard sections, 6-step inner method loop, herald-as-operator channel, 2-file closure.
...
- **Herald renamed to runner**. Not an operator channel. A top-level Claude instance that inherits G's signature as text.
- **No operator in the loop.** Invocation is a single message with a deliverable pointer; subsequent cycles are autonomous.
