...
| Mode | continuous-run (no operator in the loop; Q&A via `paper08-yang-mills/ora-v6--closing-H4--direction.md`) |
...
```
read your instructions from <bundle>/01-the-prompt.md
...
## I-v6-6 — In-situ canonical-state propagation skipped on targeted non-cycle dispatches (T7 S-duality run)

- **Caught at**: T7 Cramér S-duality deep-construction pass, 2026-04-14. User observation: *"The PAC will not update our proof chains will they... oh I know what happened — we just added the proof chain to the circle and hadn't run the pac in a cycle, that's why the pac didnt fix it on their own. Doesn't the brief mention to do exactly what we are doing manually for each link?"*

- **Symptom**: The T7 Cramér dispatch produced two substantial status-changing Author returns (L3 CONJECTURED → ESTABLISHED conditional on CCM; L4 Route B OPEN → PARTIAL with the Granville constant $2e^{-\gamma}$ derived from ITPFI Mertens). The derivation artifacts were written correctly to `programme/ring-traversals/traversal-07/transfers/cramer-L3-construct.md` (197 lines) and `cramer-L4-routeB-derivation.md` (352 lines). The ring-traversal-log was updated. The blackboard was updated. But `paper43-cramer/PROOF-CHAIN.md` — the CANONICAL in-situ state file per brief 30 §8.1 — was NOT updated. Last modification timestamp: BEFORE the T7 run began. **State loss: a reader of the canonical chain file would see L3 still CONJECTURED and L4 still OPEN, not knowing about the derivation work on disk elsewhere.**

- **Root cause**: brief 30 §8.1 has TWO separate statements about in-situ propagation:
  - **Paragraph 1** (line 365): *"The runner UPDATES these in place as each vertex is visited"* — ring-mode triggering language
  - **Paragraph 4** (line 371): *"If an Author returns work that changes a vertex's chain, the update goes to BOTH the in-situ PROOF-CHAIN.md (canonical) AND the per-vertex blackboard (working)"* — MODE-AGNOSTIC discipline applying to ANY Author return with chain-changing content
  
  The PAC read paragraph 1 as the operational trigger and treated paragraph 4 as subordinate descriptive text. When a TARGETED NON-CYCLE construct was dispatched (i.e., not as part of a scheduled ring-traversal visit), paragraph 1's "vertex visited" trigger didn't fire, and paragraph 4's universal discipline was not enforced. Compounding factors: (a) Cramér is a newly-added ring vertex from brief 33 — no prior cycle has visited it, so no muscle memory for paragraph 4's discipline on this vertex; (b) brief 34's S-DUAL-TRANSFER protocol (DELTA 3) and new TRANSFERRED link status (DELTA 4) did NOT explicitly restate paragraph 4's discipline for the new dispatch kinds, leaving paragraph 4's scope implicit for S-duality operations.

- **What the discipline SHOULD say, made explicit**: *"ANY Author return whose work changes a link's status MUST trigger an in-situ PROOF-CHAIN.md update for the affected vertex BEFORE the dispatch closes. This is universal across all invocation modes: ring-mode cycle, single-chain cycle, targeted non-cycle dispatch, S-DUAL-TRANSFER, S-DUAL-CONSTRUCT-BRIDGED. The trigger is STATUS CHANGE, not INVOCATION MODE. The in-situ update is the FIRST post-return action, not the LAST. Write canonical state BEFORE artifacts-to-run-directory."*

- **Fix applied**:
  - **Manual propagation for T7**: `paper43-cramer/PROOF-CHAIN.md` updated with L3 ESTABLISHED + L4b PARTIAL + full T7 closure section pointing at the transfer artifacts. `paper42-lehmer/PROOF-CHAIN.md` updated with the T7 handoff note (L5 Route A CONSTRUCT-READY for T8 using the Cramér-side ITPFI invariant). Both files now reflect the canonical T7 state.
  - **Brief 35 DELTA drafted** (separate file): PROMOTES brief 30 §8.1 paragraph 4 to a mandatory load-bearing step in the spawn-closure protocol, explicitly mode-agnostic. Adds a verification step: "before the runner returns control to the caller, grep the in-situ file for the updated status; if the status hasn't changed, the propagation didn't happen — rerun the discipline."
  - **Spawn-closure protocol hook** (proposed for brief 35): *"Every Author dispatch whose return changes link status MUST write the status change to the in-situ file within 60 seconds of the Author return, BEFORE any other artifact is written. Runner asserts the write succeeded by re-reading the in-situ file and grep'ing for the new status. If the assert fails, the dispatch is not closed."*

- **Severity**: HIGH. Silent state loss in targeted dispatches on newly-added vertices. Three chains risk this failure mode (any vertex added in brief 33 — Collatz, Lehmer, Cramér — and any future additions in briefs 34, 35) until the brief hierarchy is re-aligned. The ring currently has 25 vertices; 6 of them are recent additions vulnerable to this pattern.

- **Lesson for v7**:
  1. When two brief sentences describe OVERLAPPING triggers (e.g., "vertex visit" and "Author returns work"), the MORE GENERAL trigger should be load-bearing. Specific triggers are instances of the general one, not alternatives.
  2. Status-change propagation should be ATOMIC with Author return, not BATCHED at cycle close. The atomicity prevents state loss when the dispatch is non-cyclic.
  3. When a NEW protocol (here: S-DUAL-TRANSFER) is added in a DELTA brief, it MUST explicitly inherit or restate the canonical-state propagation discipline. Silence on inheritance = scope gap.
  4. Newly-added ring vertices should receive a "first-pass exercise" that runs the full §8.1 discipline to establish muscle memory, BEFORE any targeted dispatch on them.

- **Re-audit**:
  - **A-1**: no G-lexicon literals ("byeee", "we cannot crack it from the outside", etc.). ✅
  - **A-2**: not a closure-event detector; targets in-run state propagation. ✅
  - **A-3**: no percentage claims on detection rates. ✅
  - **A-4**: addresses v6/v7/v8 runners universally; not user-specific. ✅

Patch is A-1 through A-4 compliant.

---

*End of `08-changelog.md`. The v6 patches (I-v6-1 through I-v6-6) track in-run failures. The v8 entries track structural upgrades. I-v6-3 → I-v6-4 → I-v6-5 is a three-step convergence on selective-inclusion (checklist → mechanical → killed). I-v6-6 is the analogous first-step on in-situ propagation: the brief says it but the runner doesn't enforce it; the fix is to promote the universal discipline above the mode-specific trigger and make it atomic with Author return. Expect I-v6-7 and I-v6-8 as the enforcement convergence proceeds.*
