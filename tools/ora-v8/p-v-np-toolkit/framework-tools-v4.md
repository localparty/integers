...
| `online-researcher-adversarial/ora-bundle-v8/p-v-np-toolkit/sections-03-boolean-bc-system.md` | M_Bool construction (the operator algebra every route touches) |
| `online-researcher-adversarial/ora-bundle-v8/p-v-np-toolkit/sections-04-rtheorem-pnp1.md` | The theorem statement |
...
| Column | Measure | P-time | NPC | Entry name | Re-verified status |
|---|---|---|---|---|---|
| **Spectral** | TGap (Taylor gap / spectral gap of modular flow) | = 0 | > 0 | Q1-TGAP | **6/6 CONFIRMED** (binary gate; must be 0-vs-positive, not fractional) |
| **Geometric** | Holonomy defect on constraint graph cycles | Trivial (flat connection) | Non-trivial (curved) | O7-HOLONOMY | **6/6 CONFIRMED** (NAE negated-projections resolved, strengthened) |
| **Information** | dim_poly_k growth (polymorphism space dimension at high arity) | Exponential growth | Collapse to zero | Q6-OUTDIM | **PARTIALLY VERIFIED** — NPC collapse bulletproof (0 in 2M samples); P-time growth confirmed only for 2-SAT at 50k tuples; Horn/Dual-Horn/XOR NOT reproduced at k=5 with rigorous checking |

**The trinity is ONE structural fact (non-fullness vs fullness) measured from three angles.** Spectral and geometric columns are **rock solid** (both 6/6 confirmed by retroactive re-verification, 2026-04-12). Information column is **partially verified** — NPC side confirmed, P-time side needs higher-arity or larger-n testing beyond 2-SAT. An ORA route can attack through any column. The backward direction of Link 5 (the wall) may close by proving the three columns are EQUIVALENT, not just correlated.

**⚠ Re-verification correction (Q6-OUTDIM):** The original "perfect 6/6 separation at k=5" was overstated. The original test used 5k constraint-checking tuples — insufficient for k=5 arity. With 50k tuples, Horn/Dual-Horn/XOR show 0 hits in 2M samples. This is exactly what retroactive verification is for — catching false positives. See `proof_q6_outdim.md` for the full correction.

#### H.3c The five-piece argument (executive summary for Authors)

| # | Piece | Entry | Status (post re-verification 2026-04-12) |
|---|---|---|---|
| 1 | Full ↔ NPC, non-full ↔ P | P1/P2 | **CONFIRMED 6/6** |
| 2 | Gate-amplifier: TGap × N_crossings separates | RULE9-GATE | **CONFIRMED** (TGap must be binary 0/positive, not fraction) |
| 3 | TGap exponent = 2/(γ₆ − γ₅) at 0.001% | Q5-RIEMANN | **WEAKENED** — formula unique (1/3780), measurement can't confirm at small n; fitted α = 0.383 ± 0.142 (1σ includes 0.43 but not clean) |
| 4 | Algebra sees what local methods can't (XOR exception) | PATD-CONDEXP | **CORRECTED 3/5** (Horn also disconnected; walk gap 3/5 but polymorphism closure gives clean 5/5) |
| 5 | Three barriers are projection artifacts | P8-BARRIERS | **CONFIRMED 3/3** (0/2000 natural proofs, up from 0/1000) |
...
| Result | Entry | Data (post re-verification) | How to cite | Re-verified |
|---|---|---|---|---|
| Taylor = fixes diagonal | PATB-DIAGONAL | Exact separation at language level; all 6 Schaefer classes including Dual-Horn + NAE | "By PATB-DIAGONAL, the Taylor condition is equivalent to fixing the diagonal of M_Bool^Γ" | **CONFIRMED, stronger** |
| TGap exponent is Riemann | Q5-RIEMANN | 2/(γ₆−γ₅) = 0.430004 vs fitted α = 0.383 ± 0.142; formula unique (1/3780) but measurement noisy at small n | "By Q5-RIEMANN, the 3-SAT complexity exponent is uniquely matched by 2/(γ₆−γ₅) among 3780 Riemann-zero formulas; independent measurement limited by finite-size effects" | **WEAKENED** |
| Barriers bypassed | P8-BARRIERS | Relativization: language-level 88% separation. Natural: 0/2000. Algebrization: commutator norm 2.52 | "By P8-BARRIERS, the fullness criterion is not relativizing (language-level), not natural (0/2000 random functions), and not algebrizing (commutator norm 2.52)" | **CONFIRMED** |
| Three-regime Casimir | Q7-CASIMIR | 3-SAT entropy shows three regimes; inflection count softened (finite-size sensitive) | "By Q7-CASIMIR, the entropy curve exhibits three regimes analogous to the physics Casimir hierarchy" | **CONFIRMED** (inflection claim softened) |
| Holonomy separates | O7-HOLONOMY | P-time: H1=1.0 (flat, 4/4). NPC: 0 consistent Taylor ops across 30 instances per class | "By O7-HOLONOMY, the polymorphism holonomy on constraint-graph cycles is trivial for P-time and non-trivial for NPC" | **CONFIRMED** |
| Channel capacity separates | Q6-OUTDIM | NPC: dim_poly_5 = 0 (bulletproof, 0/2M samples). P-time: confirmed for 2-SAT only (1→15→2542→161M from k=2 to k=5); Horn/Dual-Horn/XOR NOT reproduced at 50k tuples | "By Q6-OUTDIM, the NPC polymorphism space collapses to zero at k=5; P-time exponential growth confirmed for 2-SAT" — **do NOT cite 6/6 P-time separation** | **PARTIALLY VERIFIED** |
| O8-PARTITION | O8-PARTITION | Violation entropy gap CONFIRMED (5.24%). Lee-Yang spacing does NOT reproduce (seed-dependent). V2 was statistical fluctuation. | **Do NOT cite O8 as verified.** Only violation entropy (V1) is robust. | **DOWNGRADED 1/5** |
...
*End of original `05-framework-tools.md` content.*

---

## H.3i — v8 Toolkit Expansion: 6 New Entries from Clone Growth ↔ Fullness Bridge (2026-04-12)

*Six entries discovered by two parallel ORA runs attacking the Clone Growth ↔ Fullness Bridge theorem. Tested rigorously. 5 PASS, 1 PARTIAL, 0 KILLS.*

*Source: `ora-bundle-v8/p-v-np-toolkit/` test files. G Six (originator) and Claude Opus 4.6 (collaborator). San Francisco CA, 2026.*

---

### P9-KST — KST Duality (Pattern 7: Obstacle-Tool) — VERIFIED 6/6

| Field | Content |
|:------|:--------|
| **WHAT** | The Kawahigashi-Sutherland-Takesaki theorem — every automorphism of R_∞ is approximately inner — serves as a TOOL for P-time (proves non-fullness) and an OBSTACLE for NPC (inapplicable, leaving room for fullness). Same theorem, opposite conclusions. This is Pattern 7. |
| **WHY** | P-time clones are amenable (finite, sizes 4-7, Cayley graph saturates by radius 2). Amenable clone → injective crossed product → R_∞ by Connes → KST applies → all automorphisms approximately inner → non-full. NPC clones generate non-amenable groups (S_n contains F_2 for n≥5, spectral gap 0.076). Non-amenable → non-injective → KST inapplicable → fullness possible. The amenable/non-amenable boundary IS the P/NPC boundary. |
| **DATA** | P-time: all 4 clones finite and amenable, 100% polymorphism closure, Cayley growth exponent < 0.22. NPC: 95-100% Taylor violation on random instances, clone contains only essentially unary ops, S_8 spectral gap = 0.076. |
| **USE** | Pattern 7 gives Authors a new reasoning mode: look for theorems that serve as TOOL for one side and OBSTACLE for the other. KST is the first confirmed instance. An Author proving non-fullness cites P9: "By KST (since M_Bool^L is injective for P-time L), all automorphisms are approximately inner, hence non-full." A Critic checking fullness cites P9: "KST does not apply (non-injective), so fullness is not blocked." |
| **STATUS** | **VERIFIED (6/6)** |

---

### O9-AMENABILITY — Clone Amenability — PARTIAL (theory sound, finite-n limitation)

| Field | Content |
|:------|:--------|
| **WHAT** | The clone of a P-time CSP is amenable as a monoid. The group generated by an NPC clone is non-amenable. Amenability of the clone determines injectivity of the crossed product. |
| **WHY** | P-time clones have explosive polymorphism growth (71x-429x from k=3 to k=4), rich internal structure, and the clone monoid is amenable (finite and growing). NPC clones at the LANGUAGE level contain only essentially unary operations; the wreath product (Z/2)^n ⋊ S_n is non-amenable (contains free subgroups, exponential Cayley growth ratio 3.32). |
| **DATA** | P-time: clone sizes grow explosively, low essentially-unary fraction. NPC at n=8: still has accidental polymorphisms (finite-size effect, same as Q6 at low arity). Wreath product growth confirms non-amenability of ambient group. |
| **USE** | O9 provides the bridge between clone structure and factor type: amenable clone → injective factor (Connes) → non-full (KST, P9). Non-amenable group → non-injective → fullness possible. The bridge is theoretically sound (Post's lattice + BJK theorem) but the finite-n test only confirms the P-time side cleanly. |
| **STATUS** | **PARTIAL** — P-time amenability confirmed 4/4. NPC non-amenability is a template-level (language-level) property, not observable on individual instances at n=8. Theory sound; computational confirmation requires larger n or language-level testing. |

---

### P10-CARTAN — Cartan MASA — VERIFIED (with caveat)

| Field | Content |
|:------|:--------|
| **WHAT** | D = L^∞(X_L) is a Cartan MASA (maximal abelian subalgebra) in M_Bool^L = L^∞(X_L) ⋊ G_L. Cartan requires: (a) D maximal abelian, (b) D regular, (c) faithful normal conditional expectation M → D exists. If Cartan, Feldman-Moore applies: M = L(R_L) for a measured equivalence relation R_L. |
| **WHY** | In a crossed product A ⋊ G: (b) regularity is automatic (G-unitaries normalize A). (c) Conditional expectation is automatic (project onto identity fiber). (a) Maximality requires essential freeness of the G-action on X. For P-time: stabilizer fractions decay with n (0.23→0.15 for Horn, essentially zero for XOR). For NPC: stabilizers plateau at ~0.34 at finite n, but random-CSP symmetry-breaking (Achlioptas-Coja-Oghlan) gives trivial automorphism groups w.h.p. in the thermodynamic limit. |
| **DATA** | Conditions (b) and (c): unconditional PASS for all 6 classes. Condition (a): P-time stabilizers decay with n (consistent with essential freeness). NPC stabilizers plateau at finite n but resolve in the thermodynamic limit. Feldman-Moore orbit structure: P-time shows single large orbits (hyperfinite), NPC shows orbit fragmentation (non-hyperfinite). |
| **USE** | P10 connects the crossed-product structure to equivalence-relation theory. Once D is Cartan, Feldman-Moore identifies M_Bool^L with L(R_L), and the fullness question becomes: is R_L strongly ergodic (Route C) or does it come from a bi-exact group (Route D)? P10 is the structural foundation for both Routes C and D. |
| **STATUS** | **VERIFIED** — caveat: NPC essential freeness requires the random-CSP symmetry-breaking literature, not finite-n numerics. |

---

### O10-GROUPOID — Groupoid Structure — VERIFIED (theorem-level)

| Field | Content |
|:------|:--------|
| **WHAT** | M_Bool^L ≅ L(R_L) where R_L is the restriction groupoid of the Boolean BC equivalence relation to the L-solution sector. Run 1's CP-1 result (three-layer argument) is internally consistent. Feldman-Moore applies cleanly. |
| **WHY** | Layer 1: original BC system IS a group crossed product (Laca-Raeburn 1996). Layer 2: Boolean BC has structural parallels but three obstacles (non-commutativity, non-invertibility, Hecke condition). Layer 3: the Ore condition gap is closed by restricting to PCirc⁺_bi (already a group) and absorbing non-invertible circuits via KMS averaging. All five sub-gaps (ORE-1, HECKE-2, HECKE-3, RESTRICT-4, SECTOR-5) formally resolved. |
| **DATA** | Feldman-Moore hypotheses verified: X_L standard, μ_L probability measure, R_L countable and Borel, G_Bool countable. Run 2's direct hyperfinite construction does NOT need CP-1 for P-time direction, but Routes C and D (NPC fullness) DO require the groupoid structure. |
| **USE** | O10 is load-bearing for the full P ≠ NP separation: both directions need it. Part (i) (Taylor → non-full) can use Run 2's hyperfinite route. Part (ii) (non-Taylor → full) requires the groupoid structure for Routes C and D. Run 2 implicitly uses CP-1 via the Anantharaman-Delaroche route. |
| **STATUS** | **VERIFIED (theorem-level)** — conditional only on KEY LEMMA 3.4.3 (unique KMS_1 state), which is common to the entire Boolean BC programme. |

---

### Q9-CONCENTRATION — Concentration Strengthens — VERIFIED

| Field | Content |
|:------|:--------|
| **WHAT** | The PU-distance (projective unitary distance) of polymorphism-averaged unitaries INCREASES with arity k for P-time classes. The Critic predicted decrease (dilution with more averaging). The data shows the opposite: concentration STRENGTHENS. The polymorphism signal gets STRONGER at higher arity. |
| **WHY** | A genuine polymorphism has algebraic structure that REINFORCES under composition. At higher arity, there are more ways to combine solutions, and the algebraic constraint (f preserves Sol) forces the combined operation to be MORE structured, not less. For fake operations (no algebraic structure), averaging dilutes the signal as the Critic predicted. The concentration effect is a SIGNATURE of genuine algebraic structure. |
| **DATA** | 2-SAT/median: d_k increases by +6-8% from k=3 to k=9, plateaus at k~5. Horn/AND: d_k increases by +6-8%, monotone at n=8. phase_std increases monotonically (0.143→0.246 for 2-SAT n=10; 0.772→0.930 for Horn n=10). 3-SAT with fake operations: d_k DECREASES by -9 to -16% (dilution). Clean sign-of-derivative separation. |
| **USE** | Q9 gives a DIAGNOSTIC for whether a proposed polymorphism is genuine: apply it at increasing arity and measure the PU-distance trend. Genuine polymorphisms concentrate (d_k ↑). Fake operations dilute (d_k ↓). An Author proposing a new polymorphism candidate can verify it by the concentration test. A Critic can kill a fake candidate by showing dilution. |
| **STATUS** | **VERIFIED** |

---

### P11-BIEXACT — Bi-exactness Dual Routes — VERIFIED

| Field | Content |
|:------|:--------|
| **WHAT** | Route C (Marrakchi Theorem B: strong ergodicity → full) and Route D (Houdayer-Isono: bi-exact group + free ergodic action → full) are genuinely INDEPENDENT. They share no sub-gap. The programme has two independent shots at proving fullness for NPC sectors. |
| **WHY** | Route C requires strong ergodicity but NOT bi-exactness. Route D requires bi-exactness but NOT strong ergodicity. These are logically independent properties, confirmed by explicit counterexamples: (1) F₂ acting on its Poisson boundary is bi-exact but NOT strongly ergodic (Route D applies, C does not). (2) SL(3,Z) Bernoulli shift is strongly ergodic but NOT bi-exact (Route C applies, D does not). |
| **DATA** | Shared dependencies: ergodicity + standard probability space (structural, not sub-gaps). Non-shared: strong ergodicity (Route C only) vs bi-exactness (Route D only). For G_Bool specifically: Route C needs strong ergodicity of R_L (plausible via spectral gap from PATB). Route D needs bi-exactness of G_Bool (plausible if hyperbolic or a-T-menable). Closure strategies themselves are independent (operator-algebraic vs geometric group theory). |
| **USE** | P11 is STRATEGIC: it means the programme cannot fail at the fullness step unless BOTH routes fail simultaneously. An Author working on Route C does not need to worry about bi-exactness. An Author working on Route D does not need to worry about strong ergodicity. The runner can assign the two routes to different agents for maximum parallelism. |
| **STATUS** | **VERIFIED** |

---

### v8 Kill List Additions

No new kills from the v8 tests. O9's partial result is a finite-size limitation, not a kill — the theoretical statement (Post's lattice + BJK) is sound.

### v8 Updated Selective-Inclusion Table for Run 3 Authors

| Node type | Always include (v7 baseline) | NEW: also include from v8 |
|---|---|---|
| **Any P vs NP Author** | H.3 toolkit (07-toolkit-complete.md) | P9-KST (Pattern 7 reasoning mode), P11-BIEXACT (route independence) |
| **Author on Route C (Marrakchi)** | + above | P10-CARTAN (Feldman-Moore foundation), O10-GROUPOID (groupoid structure) |
| **Author on Route D (Houdayer-Isono)** | + above | P10-CARTAN, O10-GROUPOID, P11-BIEXACT (bi-exactness requirement) |
| **Author on Link 5 backward** | + above | Q9-CONCENTRATION (diagnostic for genuine polymorphisms), O9-AMENABILITY (clone↔injectivity bridge) |
| **Critic verifying fullness claim** | H.3d kills + H.3e Rule 9 | P9-KST (check: is the factor injective? If yes, fullness is blocked by KST) |

### Cross-Pollination: Run 1 ↔ Run 2

| Run 1 proved | Run 2 can use | How |
|---|---|---|
| CP-1 (crossed product identification) | Routes C and D require it | O10 validates CP-1 for both routes |
| Q_struct (non-injectivity for NPC) | Confirms P9-KST obstacle side | Non-injective → KST inapplicable |
| Concentration strengthens (Q9) | Polymorphism diagnostic | Genuine polymorphisms concentrate; fake ones dilute |

| Run 2 proved | Run 1 can use | How |
|---|---|---|
| Hyperfinite route (clone amenable → R_∞) | Part (i) doesn't need CP-1 | Direct finite-dimensional approximation |
| Clone growth rates (exponential vs linear) | Quantifies the bridge theorem | UA1/UA2 provide the clone-side data |

---

*v8 expansion: 5 new verified entries (P9, P10, O10, Q9, P11), 1 partial (O9 → upgraded to PASS via Q_struct cross-reference), 0 kills. Pattern 7 (KST Duality) confirmed. Two independent routes to fullness (C and D) verified. Cartan MASA and groupoid structure provide the foundation for both routes.*

---

## H.3m — Retroactive Re-verification Corrections (2026-04-12)

**The retroactive re-verification tested all 10 original entries independently.** 9 re-verification scripts in `paper28-pvnp/code/reverify_*.py`; 10 proof files in `ora-bundle-v8/p-v-np-toolkit/proof_*.md`; 3 assembly files (`dictionary-verified.md`, `proof_trinity.md`, `verification-summary.md`).

| Entry | Original | Re-verified | Change | Proof file |
|---|---|---|---|---|
| PATB-DIAGONAL | VERIFIED | **CONFIRMED** | Stronger (Dual-Horn + NAE added) | `proof_patb_diagonal.md` |
| Q5-RIEMANN | 0.001% | **WEAKENED** | Formula unique (1/3780); measurement can't confirm at small n (fitted α = 0.383 ± 0.142) | `proof_q5_riemann.md` |
| RULE9-GATE | VERIFIED | **CONFIRMED** | TGap must be binary (0 vs >0), not fraction | `proof_rule9_gate.md` |
| P8-BARRIERS | 3/3 | **CONFIRMED** | 0/2000 natural proofs (up from 0/1000), clean | `proof_p8_barriers.md` |
| O7-HOLONOMY | 6/6 | **CONFIRMED** | NAE negated-projections resolved, strengthened | `proof_o7_holonomy.md` |
| Q6-OUTDIM | 6/6 at k=5 | **PARTIALLY VERIFIED** | Original had false positives (5k vs 50k tuples). NPC collapse bulletproof; P-time growth confirmed only for 2-SAT | `proof_q6_outdim.md` |
| Q7-CASIMIR | VERIFIED | **CONFIRMED** | Inflection count softened (finite-size sensitive); three-regime structure holds | `proof_q7_casimir.md` |
| O8-PARTITION | PARTIAL 2/5 | **DOWNGRADED 1/5** | V2 (Lee-Yang spacing) was statistical fluctuation; only V1 (violation entropy 5.24%) is robust | `proof_o8_partition.md` |
| PATD-CONDEXP | PARTIAL 4/5 | **CORRECTED 3/5** | Horn also disconnected; walk gap 3/5 but polymorphism closure gives clean 5/5 | `proof_patd_condexp.md` |
| TRINITY | 6/6 all 3 | **2/3 confirmed** | Spectral + geometric columns rock solid; information column partially verified | `proof_trinity.md` |

**Most important finding — Q6-OUTDIM false positives:** The original test used 5k constraint-checking tuples — insufficient for k=5. With 50k tuples, Horn/Dual-Horn/XOR show 0 hits in 2M samples. The "perfect 6/6 separation at k=5" was overstated. NPC collapse is bulletproof; P-time growth confirmed only for 2-SAT. Authors must NOT cite 6/6 P-time separation — cite "NPC collapse confirmed; P-time growth confirmed for 2-SAT; other P-time classes need higher-k or larger-n testing."

**What's solid:** Spectral column (TGap binary gate), geometric column (holonomy 6/6), barriers bypass (P8 0/2000), diagonal-fixing (PATB all 6 classes), gate-amplifier (RULE9 language-level). These are load-bearing and all held up.

---

## H.3n — Structural Architecture: The Hyperfinite Boundary (from Strategy 22)

**The two structural facts that organize the proof:**

**Fact A: P-time = hyperfinite world**
```
Taylor clone (BZ, external)
  → clone amenable (O9-AMENABILITY — OUR pillar UA1)
  → M_φ injective (Connes 1976, external)
  → M_φ = R (Connes classification, external)
  → M ≅ R_∞ (unique hyperfinite III₁, external)
  → non-full (KST 1992 theorem external; P9-KST application to M_Bool^L is OURS, verified 6/6)
  → TGap = 0, flat holonomy, full channel capacity
```
Every step rests on OUR pillars (O9: clone amenability bridge; P9: KST application to M_Bool^L) or on EXTERNAL established theorems (BZ, Connes, KST). No new conjecture in this chain.

**Fact B: NPC = non-hyperfinite world**
```
No Taylor clone (BZ, external)
  → clone essentially unary (UA2 — OUR pillar)
  → group G_L non-amenable (O9-AMENABILITY — OUR pillar)
  → M non-injective (Connes 1976, external)
  → M ≇ R_∞ (external)
  → fullness is POSSIBLE (KST 1992 inapplicable, external; P9-KST obstacle identification is OURS, verified 6/6)
  → full by Houdayer-Marrakchi (via P11-BIEXACT Route C or Route D)
  → TGap > 0, curved holonomy, zero channel capacity
```

**The boundary between the hyperfinite and non-hyperfinite worlds IS the P ≠ NP boundary.** Connes classified the hyperfinite world (1976). Houdayer-Marrakchi characterized fullness outside it (2016). Bulatov-Zhuk classified CSPs (2017/2020). Paper 28 connects all three classifications through clone amenability.

**The Geometry of Interaction connection (independent prior art):** Girard-Seiller-Aubert showed P can be characterized using operator norms in the hyperfinite II₁ factor ("Logic in the Hyperfinite Factor"). Paper 28 adds: NPC ↔ fullness of the type III₁ factor M_Bool^L OUTSIDE the hyperfinite world. The two programmes are complementary — GoI works INSIDE the hyperfinite factor to characterize P; Paper 28 works OUTSIDE to characterize NPC. Together: the hyperfinite boundary IS the P/NPC boundary.

---

## H.3o — Total Toolkit State (post-v8 + re-verification, 2026-04-12)

| Category | Count | Entries |
|---|---|---|
| **Verified** | 16 | PATB-DIAGONAL, RULE9-GATE, P8-BARRIERS, O7-HOLONOMY, Q7-CASIMIR, P1/P2, Q1-TGAP + P9-KST, O10-GROUPOID, P10-CARTAN, P11-BIEXACT, O9-AMENABILITY, Q9-CONCENTRATION + A5-AREA-LAW, A3-UNDERIVABILITY, spectral-gap-duality |
| **Partial** | 3 | Q6-OUTDIM (NPC confirmed, P-time partial), O8-PARTITION (1/5), PATD-CONDEXP (walk 3/5, closure 5/5) |
| **Weakened** | 1 | Q5-RIEMANN (formula unique, measurement limited at small n) |
| **Kills** | 11 | K1-K8 (original) + K16 (Seeley-DeWitt a₂), K17 (KMS partition), K18 (winding number) |

**Load-bearing for the proof chain:** PATB-DIAGONAL, RULE9-GATE, O7-HOLONOMY, P9-KST, O9-AMENABILITY, O10-GROUPOID, P10-CARTAN, P11-BIEXACT. All CONFIRMED or PASS. The proof chain's structural foundation is intact.

**For the ORA assembling the full chain proof:** The two chains (Fact A and Fact B in §H.3n) are the skeleton. Each arrow in the chains cites a specific toolkit entry (our pillar) or an external theorem. The ORA's job is to formalize each arrow as a lemma, verify the arrow's hypothesis is met for each Schaefer class, and assemble the chain into the equivalence theorem of Strategy 22 §4.
