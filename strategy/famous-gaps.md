...
| **Collatz Conjecture** | **paper41-collatz** *(new 2026-04-14)* | The e-circle's **third face: HARMONICS**. The Collatz map alternates between the 2nd and 3rd harmonics ($\mu_2$, $\mu_3$) of the e-circle. Conjecture = all harmonics decay to fundamental ($n=1$). **Cuntz $\mathcal{O}_2$ formulation** (Mori 2024, published Springer 2025): Collatz $\iff$ no non-trivial reducing subspaces. $\mathcal{O}_2 \hookrightarrow A_{BC}$ via $p=2$ Hecke sector. **Lyapunov $\approx 0$ = KMS$_1$ critical point**; average $\log(3/4) < 0$ (contracting). **ITPFI**: $\lambda_2 = 1/2 > \lambda_3 = 1/3$ → p=2 beats p=3. Type III$_1$ eliminates cycles. Wall: +1 additive shift (shared with Goldbach). | **3/7 links closed. Confidence 4/10** *(upgraded after three-face picture + Lyapunov=KMS₁ + ITPFI dominance)*. |
