...
*Sibling to `paper12/26-convergence-prompt.md` (the empirical*
...
emits the two output artifacts to `paper12/research/` with the
...
The prompt is sibling to `paper12/26-convergence-prompt.md`. The
two prompts answer different questions:

| Prompt | Question | Cadence |
|:--|:--|:--|
| `paper12/26` | "Is the framework still consistent with reality as data improves?" | Once per major experimental release |
| `paper12/27` (this) | "Is the framework structurally minimal, and how precise can theory predict observables?" | Once per Clay-proof refinement, OR on demand |
...
9. paper12/research/sigma-exp-table.md (built by paper12/26 round 1)
   READ FOR: the 36 master-table observables with their σ_exp.

10. paper12/research/sigma-exp-table-tier-C.md (build if missing)
...
Output the graph to:
  paper12/research/{N}-postulate-relaxation-round-{R}-graph.md
...
CONVENTIONS:
  - Match register from paper12/research/24 ("the moment") and
...
## What this prompt does that paper12/26 does NOT do

`paper12/26` watches the framework as experimental data improves —
its job is to tally σ-distances on a fixed framework structure as
new measurements come in. It is **empirically reactive**.

`paper12/27` (this prompt) tests the framework's structural
...
- **Single biggest broken assumption**: likely the m_Z and v
  bookkeeping flag from paper12/26 rounds 1-4 (research/23 carries
  raw single-zero formulas superseded by research/154 Laurent
  shift). This is the same data port that paper12/26 has been
...
round 2 (similar to how paper12/26 needed 4 test rounds to converge).
...
test methodology used for paper12/26). The next move after committing
...
(a) **Test loop**: launch a single test agent in dry-run mode,
    ...
    until quality ≥ 9. Same methodology as `paper12/26-convergence-prompt.md`
...
significantly more complex than `paper12/26` (5 phases, 5 layers,
